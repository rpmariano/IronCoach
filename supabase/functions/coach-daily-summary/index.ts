...
const MEAL_DOCTRINE =
  ...
  `- BAIXO ATRITO, a regra mais importante: sugestões por CATEGORIA e quantidade redonda ` +
  `("150g de peixe", "2 ovos"), nunca um cardápio de precisão que varia todos os dias — ` +
  `reutiliza um núcleo pequeno de alimentos comuns, o atleta não pode sentir que precisa de ` +
  `ir às compras por um ingrediente novo todos os dias.\n` +
  ...
  `ovo 12,5 (≈6g/ovo), skyr/grego 0% 10-12, tofu firme 12-15, lentilhas/grão 8-9. Valores só ` +
  `como referência de ordem de grandeza — arredonda para porções redondas, não fechar ao grama.\n` +
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const lastWeekPlan = isMonday && lastWeekItems.length > 0
      ...
      : null;
    // A meteorologia da prova, se for nos próximos 7 dias (ação 4.2), o que
    // ela prescreveu nos últimos 14 dias face ao que aconteceu (Fase 3) e o
    // que o atleta já viu na app (5.1): as boas-vindas ao abrir, os avisos e
