...
Deno.test("regra 5(c) cobre o período do plano ativo (não um sub-período curto), com teto de 10 dias por limite de tokens", () => {
  ...
  // todo, não um excerto. O teto de 10 dias existe só como salvaguarda
  // técnica para planos excecionalmente longos, não como o comportamento
  // normal.
  const sys = buildSystemInstruction(
    null,
    { ...BIO_BASE, coach_can_set_nutrition_goals: true },
  ...
  assertStringIncludes(sys, "se esse período tiver MAIS de 10 dias a partir de hoje, cobre só os primeiros 10 dias");
});
