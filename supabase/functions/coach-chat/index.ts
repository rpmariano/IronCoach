...
export function buildSystemInstruction(
  ...
  `- Por isso: assim que o atleta revelar um facto duradouro, GUARDA-O com save_coach_note em vez de contares com o histórico para o recordar. É o que impede que voltes a propor daqui a duas semanas exatamente o que ele já disse que não quer.\n` +
    `- Por isso também: antes de pedires ao atleta detalhes sobre algo que ele acabou de dizer que registou ou criou (uma prova, um treino, uma refeição, uma avaliação corporal, um par de sapatilhas...), verifica primeiro se já consta nos DADOS ESTRUTURADOS acima — a gravação na BD acontece antes da mensagem chegar até ti, por isso os dados já lá estão quase sempre. Só perguntes o que realmente não conste no contexto; pedir de novo o que já está à vista é o erro mais visível que podes cometer aos olhos dele.\n` +
