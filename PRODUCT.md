...
- Botões de Sistema & Ergonomia: Alvo tátil mínimo obrigatório de **44px × 44px**, superfícies neutras para botões de navegação/ações secundárias e botão flutuante (FAB) de 56px em gradiente âmbar (`#d97706` → `#fbbf24`, inline em `Layout.jsx`) — decisão de marca deliberada, não usa `var(--accent)`. (Substitui a decisão anterior de creme `#f3d5ab`/`var(--fab-bg)`, documentada até 2026-08-30 — ver adenda em `.impeccable/critique/ui_audit.md` §2.9.)
