...
| 8 — Nível por prova / trail | ⚠️ Parcial — `categorizeElevationRatio()`, invalidação do nível em `RunAgenda.jsx`, e agora `assessRaceLevelTriage()` (motor de triagem por Tempo em Pé + D+, `raceLevelTriage.ts`) implementados e testados; falta LIGAR o motor a um consumidor real (UI/coach-chat) e a tabela do help por categoria em `ExperienceLevelHelp.jsx` — ver [specs/nivel-por-prova.md](../../specs/nivel-por-prova.md) |
