...
export default function FirstDayCard({ firstName, onTalk, onCreateRace }) {
  return (
    <div className="rounded-[24px] shrink-0" style={{ background: 'rgba(34,211,238,.08)', border: '1px solid rgba(34,211,238,.32)', padding: 20, boxShadow: 'var(--shadow-card)' }} data-testid="first-day-card">
      ...
      <button type="button" onClick={onTalk} className="w-full inline-flex items-center justify-center gap-2 min-h-[46px] mt-[15px] rounded-[12px] text-[13.5px] font-extrabold" style={{ background: 'var(--grad-coach)', color: 'var(--coach-ink)' }}>
        <Sparkles size={16} /> Falar com a Carol
      </button>
      <button type="button" onClick={onCreateRace} className="w-full min-h-[44px] mt-[9px] rounded-[12px] text-[13px] font-bold" style={{ background: 'transparent', border: '1px solid rgba(255,255,255,.16)', color: 'var(--text-3)' }}>
