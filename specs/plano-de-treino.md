...
| `coach_plan_items.meal_macros jsonb` (2026-09-05: tentado e revertido DUAS vezes; causa isolada e corrigida a 2026-09-06) | `meal_items[]` (refeições) + `meal_estimated_kcal/protein_g/carbs_g/fat_g`, campos **IRMÃOS** de `meal_suggestion`/`meal` e todos **opcionais** — a Carol pensa em alimentos/gramas concretos para calcular macros reais, mas o texto de `meal_suggestion` continua a versão generalizada de sempre, extraída de forma totalmente independente. Alimenta os anéis de `WeeklyPlanCard.jsx` com a estimativa real da sugestão (rótulo "Estimativa desta sugestão"); sem `meal_macros` válido, o frontend cai no objetivo diário do perfil. **CAUSA REAL DOS DOIS INCIDENTES (isolada experimentalmente contra a API real a 2026-09-06):** a API de function calling do Gemini rejeita, com `400 INVALID_ARGUMENT` e sem indicar campo, um **ARRAY com limites de comprimento aninhado dentro de OUTRO ARRAY com limites** — aqui, `meal_items` (`minItems:2`/`maxItems:6`) dentro de `save_meal_suggestions.suggestions` (`minItems:1`/`maxItems:14`). Prova: dois payloads com **bytes, nós, propriedades, enums e profundidade IDÊNTICOS** (15.684 B cada) deram resultados opostos — o mesmo bloco passa (200) dentro do array **sem** limites do `propose_training_plan`, e falha (400) dentro do array **com** limites do `save_meal_suggestions`. Isso elimina tamanho, profundidade, contagem de nós, `enum`, número de declarações e orçamento agregado (confirmado ainda por: baseline com enchimento inerte até aos mesmos 17.394 B → 200; remover uma declaração inocente → continua 400; contents triviais → continua 400, logo é stateless). **Fix:** tirar `minItems`/`maxItems` de `meal_items`. O mínimo de 2 refeições passa a ser imposto só em código, por `buildMealMacros()`. Payload final validado a 200 contra a API real com as 8 declarações e ~47k tokens de contexto. **Porque é que a validação da 2ª tentativa falhou:** testou o `PROPOSE_PLAN_TOOL` isolado, cujo array `items` **não tem limites** — ou seja, o único sítio onde o bug não podia acontecer. Nunca enviou o `save_meal_suggestions`. **Nota importante sobre o `buildTools()`:** ele NÃO envia as 8 declarações em todas as mensagens — filtra por caso de turno (`classifyTurn`/`allowedToolsFor`). Só o caso E (mensagem normal) envia o `save_meal_suggestions`, e é por isso que os incidentes pareceram intermitentes: as mensagens que passaram eram respostas a decisões (casos D/F_PLAN), que enviam menos declarações. Guarda em CI: `index.test.ts` tem agora um teste estrutural que percorre `buildTools(allowedToolsFor(caso))` nos 7 casos de turno e falha se algum array com limites contiver outro array com limites (verificado a falhar quando os limites são repostos). |
