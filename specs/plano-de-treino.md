...
| `coach_plan_items.meal_macros jsonb` (2026-09-05, **tentado e revertido no mesmo dia**) | Tentativa: texto generalizado + `estimated_kcal/protein_g/carbs_g/fat_g` calculados pela Carol a partir de alimentos/gramas concretos, para os anéis de `WeeklyPlanCard.jsx` mostrarem a estimativa real da sugestão em vez do objetivo diário. **Revertido em produção horas depois**: o schema aninhado (`items[]` de objetos + 4 campos numéricos, dentro de um `OBJECT` já dentro do `ARRAY` de `propose_training_plan`) fazia a API de function calling do Gemini devolver `400 INVALID_ARGUMENT` em **qualquer** mensagem ao Coach, não só nas que usariam a tool — as tools vão sempre todas na chamada, independente da que o modelo escolhe. Coluna `meal_macros` fica na BD (aditiva, inofensiva, sempre `null` agora) e o frontend já degrada em segurança para o objetivo do perfil quando ausente — não houve necessidade de reverter isso. Antes de retomar: validar um schema mais raso (menos níveis de nesting) contra a API real do Gemini antes de o levar a produção. |
