...
- Não é preciso fallback de 404: a navegação é estado do Zustand mais um parâmetro `?tab=` (uma notificação ou um link), não rotas por caminho. As recargas técnicas (atualização, ecrã que falhou a carregar) usam `?resume=<separador>`, que o arranque aplica sem o tratar como notificação e tira da barra de endereço (`src/lib/appUpdate.js`).
