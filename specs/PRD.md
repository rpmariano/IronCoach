...
- **Registo de nova refeição — mesmo padrão da Corrida (3.4): um único cartão, forma de introdução à escolha**: os campos comuns (data, tipo de refeição, observações) ficam sempre visíveis; um seletor "Como queres registar?" (Foto/IA por omissão, ou Manual) decide o resto do cartão. **As duas formas passam pelo Coach** — não há caminho de registo sem análise.
  ...
  - **Editar uma refeição existente** (botão "Editar" em `MealCard.jsx`, ao lado de "Eliminar") é sempre pelos campos e não passa pelo Coach: só permite alterar data/tipo/observações e o nome/gramas dos alimentos já gravados, ou removê-los — nunca acrescentar um alimento novo (isso exigiria estimar os valores nutricionais dele, o que só a Edge Function faz). Sem seletor Foto/Manual nem chamada nenhuma a `analyze-meal`; é um `update`/`delete` direto em `meals`/`meal_items`.
...
- **Registo de nova sessão — mesmo padrão da Corrida/Nutrição/Corpo: um único cartão, forma de introdução à escolha**: os campos comuns (tipo Força/Aula, data, nome, grupos musculares/modalidade, observações) ficam sempre visíveis; um seletor "Como queres registar?" (Foto/IA por omissão, ou Manual) decide o resto do cartão. **As duas formas passam pelo Coach** — não há caminho de registo sem análise.
  ...
  - **Editar uma sessão existente** (botão "Editar" no cartão, ao lado de "Eliminar") é sempre pelos campos e não passa pelo Coach: além das métricas/nome/categorias/observações, é também o único sítio onde se gerem exercícios e séries por completo (adicionar/remover exercícios, adicionar/remover séries, editar reps/carga) — o cartão do treino deixou de ter essa opção (nunca funcionava: usava `workout_session_id`, uma coluna que não existe; a coluna real é `session_id`). Ao gravar, substitui-se sempre a lista de séries toda (`delete` + `insert`), nunca um merge. **Não há ação "Reanalisar"** (repescar os prints e voltar a extrair por IA) — editar já cobre corrigir exercícios/séries à mão, ao contrário da Corrida/Refeição, onde "Reanalisar" continua a existir por ser a única forma de voltar a ler os prints.
...
- **Agenda de provas (`race_events`) — só o formulário de `RunAgenda.jsx` cria/edita.** `RunRegistration.jsx` chegou a ter um segundo formulário para o mesmo efeito (`mode: 'prova'`), nunca alcançável por nenhum ecrã (`initialMode` era sempre `'corrida'`) — removido nesta reorganização em vez de deixado a apodrecer a par do real.
  - **Campos e obrigatoriedade**: só "Site da prova" e "Notas" são opcionais — todos os outros (data, local, nome, tipo, distância, tempo-alvo, ritmo-alvo, e D+ quando o tipo é trail) são obrigatórios, reforçado com `NOT NULL`/`CHECK` na BD, não só no formulário.
