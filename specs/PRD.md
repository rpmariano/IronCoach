...
- Não é preciso fallback de 404: a navegação é estado do Zustand mais um parâmetro `?tab=` (uma notificação ou um link), não rotas por caminho. As recargas técnicas (atualização, ecrã que falhou a carregar) usam `?resume=<separador>`, que o arranque aplica sem o tratar como notificação e tira da barra de endereço (`src/lib/appUpdate.js`). O separador e o ecrã de topo abertos (registo, edição, prova, "O plano") guardam-se em `localStorage` (`ironcoach:ecra-aberto`, `src/utils/navigationRestore.js`): se o Android matar a app, voltar em menos de 30 minutos repõe-nos, e o registo reabre com o seu rascunho. Não repõe com uma notificação acabada de tocar (`?carol=`), uma bancada pedida por URL ou um `?tab=` sem nada a meio; um registo já gravado (confirmação à vista) conta como fechado. Um ecrã de topo aberto também adia a atualização automática.
...
- **Plano para o dia da prova (decidido 2026-09-12, ver `specs/plano-de-prova.md`)**: tabela km a km (ritmo, passagem, instrução) pela fórmula partilhada `racePacing`, ajustada aos troços do percurso quando o site da prova os descreve (`web_info.route_segments`); cartão no hub nos últimos 7 dias, a Carol apresenta-o na véspera e cita o primeiro km na manhã, e no balanço compara os parciais registados com o plano.
...
- **Registo de nova corrida — um único cartão, forma de introdução à escolha**: os campos comuns (Treino/Competição, tipo de treino ou disciplina, data, RPE, nome) ficam sempre visíveis; um seletor "Como queres registar?" (Foto/IA por omissão, ou Manual) decide o resto do cartão. **As duas formas passam pelo Coach** — não há caminho de registo sem análise.
  - **Foto (IA)**: 1 a 6 prints da app de corrida (Strava, Garmin, ...), comprimidos e normalizados para JPEG no cliente (`src/lib/image.js`), enviados para `analyze-run`. Extrai distância/duração/splits/métricas do relógio e gera a nota do Coach (`coach_notes`).
    - **Métricas em falta** (2026-09-24): a análise **já gravou** a corrida quando o aviso aparece. Daí em diante, tudo o que o atleta fizer atualiza **essa** corrida, nunca cria outra: "Mais prints" abre o seletor e a análise seguinte é uma reanálise (`run_id`, `keep_paths` dos prints que já foram, `images` só os novos, e os campos que não vêm da imagem); "Manual" grava em `mode: "manual"` com o mesmo `run_id`; sem mudanças nos prints nem nos campos, fecha com a corrida como está.
    - **Gemini ocupado** (500/502/503/504): a função repete até 3 vezes (esperas de 2, 4 e 7 s), dentro de prazos contados desde o pedido (extração 60 s, comentário 88 s). A app espera 130 s (`ANALYZE_TIMEOUT_MS`, `src/lib/edgeTimeouts.js`), acima desse prazo, para nunca desistir de uma corrida que o servidor ainda vai gravar — o "Tentar de novo" duplicá-la-ia. A regra é a mesma na refeição, no ginásio, no corpo e no diploma (`supabase/functions/_shared/geminiFetch.ts`).
    - **Rascunho**: a corrida já gravada fica no rascunho (`createdRun`) e os prints que foram com ela passam a ser dela (`{ path }`, como a editar) — se o Android matar a app com o seletor aberto, reabrir continua sobre a mesma corrida. Ao fechar a corrida (foto, "Prosseguir" ou manual), o `createdRun` sai logo do rascunho.
    - **Só o nome, a data ou as sapatilhas** mudados depois do aviso → `update` direto, sem nova leitura dos prints; RPE, notas ou tipo → reanálise.
  ...
  - **Editar uma corrida existente** é pelos campos (o seletor fica escondido) e pelos prints: o bloco "Prints do relógio" deixa remover e juntar prints (2026-09-13). Ao gravar há três caminhos, decididos por comparação com o estado inicial:
    - **Prints removidos ou juntados** → reanálise pelas imagens: `analyze-run` com `run_id` sem `mode`, `keep_paths` (só caminhos que a corrida já tinha) e `images` novas; a função lê o conjunto final, regrava distância/duração/métricas, guarda-o como `photo_paths`, apaga do bucket os que saíram e regenera `coach_notes`. Nome, data, RPE, sapatilhas, notas e tipo vão junto. Os números escritos à mão nessa edição são substituídos pelo que as imagens dizem. Enquanto as signed URLs dos prints não carregam, não há prints "removidos" (senão guardar apagava-os).
    ...
    - **"Reanalisar"** no cartão da corrida usa o mesmo modo de reanálise com os prints guardados e, desde 2026-09-13, também regenera `coach_notes`; devolve erro claro em corridas sem fotos.
...
- **Agenda de provas (`race_events`) — só o formulário de `RunAgenda.jsx` cria/edita.** `RunRegistration.jsx` chegou a ter um segundo formulário para o mesmo efeito (`mode: 'prova'`), nunca alcançável por nenhum ecrã (`initialMode` era sempre `'corrida'`) — removido nesta reorganização em vez de deixado a apodrecer a par do real.
  ...
  - **Subida total (D+, desnível acumulado, `elevation_gain_m`)** — no ecrã chama-se "Subida total (m)" desde 2026-09-23 — só aparece e só é pedido quando o tipo é `trail` — trocar para `estrada` limpa o campo. A BD reforça a regra com `CHECK (race_type = 'trail' OR elevation_gain_m IS NULL)`.
...
- **Data de nascimento** (tab Pessoal): guarda-se `profiles.birth_date`, **nunca a idade**. Uma coluna com a idade fica silenciosamente errada no primeiro aniversário; a idade deriva-se em runtime com `ageFromBirthDate()` (`src/utils/body.js`). O ecrã mostra a idade calculada ao lado do rótulo, como confirmação de que a data está certa.
  ...
  - A função está duplicada em dois runtimes (cliente e `coach-chat`) porque não partilham módulos. Os dois têm comentário cruzado: mexer num obriga a mexer no outro. (Era em três até `suggest-goals` ser removida a 2026-08-23 — ver 3.7 abaixo.)
...
- **Prazo do arranque (2026-09-24).** Um pedido (`coach_plan_items`) ficou 37 s preso na rede do telemóvel e a app ficou esse tempo no logo: `loadInitialData` esperava por tudo sem limite. Agora espera no máximo `INITIAL_LOAD_BUDGET_MS` (10 s). O que não chegou continua em segundo plano e entra quando chegar, e até lá `dataPending` fica verdadeiro (só no primeiro carregamento de cada conta). O Início (primeiro dia), o onboarding e as boas-vindas não tiram conclusões de listas vazias enquanto `dataPending`. Numa recarga da mesma conta, um pedido que falha ou chega atrasado mantém o que lá estava; ao mudar de conta, as listas da anterior limpam-se logo. As chamadas do arranque, da sessão e do regresso à app juntam-se a um carregamento em curso (`{ join: true }`); as de depois de gravar um registo são sempre um carregamento novo, e o mais recente ganha, fatia a fatia (um erro no mais recente também impede o anterior de escrever). O `dataPending` nunca passa de `DATA_PENDING_MAX_MS` (45 s); fechar o ecrã reposto e abrir a notificação esperam por `whenDataReady()`. As listas só se limpam ao mudar de conta — no arranque a frio, um set de listas vazias fazia o `dailySummaryRefresh` tomá-las como ponto de partida e cada abertura pedia um resumo novo ao modelo; o observador espera agora por `trainingLoadedFor` (revisão pré-deploy de 90bfa9b).
- **Prazo do chat (2026-09-24).** Uma atleta ficou sem resposta: o `coach-chat` dava 40 s a cada chamada ao Gemini e repetia do zero (40 + 40 s), e a resposta — um pedido para reorganizar a semana, ~51 mil tokens de contexto — precisava de mais. Agora: uma tentativa de até `GEMINI_ATTEMPT_MS` (100 s), repetição só quando o Gemini diz que está ocupado (503, com esperas crescentes — `_shared/geminiFetch.ts`, o mesmo dos registos), tudo dentro de `CHAT_BUDGET_MS` (125 s por pedido, com folga para o limite de ~150 s da plataforma); uma ronda de ferramentas só começa se ainda couber uma tentativa útil. As falhas ficam nos logs do servidor (antes só a app registava o timeout). A app continua a desistir da resposta síncrona aos 45 s e a sondar `coach_messages`, mas pára quando lê duas vezes seguidas o lock (`profiles.coach_chat_busy_since`) livre sem resposta gravada — antes esperava sempre os 3 minutos. Com isto (revisão pré-deploy de bdc93cf): o lock só é órfão ao fim de `CHAT_BUDGET_MS + 35 s` (era 120 s, menos do que um pedido pode durar) e cada pedido só liberta o seu (`.eq("coach_chat_busy_since", lockIso)`) — o comentário "mais de 2 minutos" da migration 20260820120000 ficou desatualizado; uma falha de rede rápida ao Gemini repete-se uma vez (só a lentidão não se repete); e se uma ferramenta de escrita já correu (plano proposto, objetivos, prova, intervenção) e falta tempo para o texto, grava-se uma frase curta ("Deixei-te a proposta de plano no Início…") em vez de acabar sem resposta.
