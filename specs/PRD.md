...
- **Dívida registada — a mesma análise por IA não está ligada em Ginásio.** Os botões "Analisar"/"Registar com foto" desse módulo são placeholders (`alert(...)` ou `disabled`), e o registo manual não gera comentário do Coach nenhum (ao contrário de Corrida, Nutrição e Corpo, que já geram nos dois caminhos — ver 3.2, esta secção e 3.5). A Edge Function `analyze-gym` existe e continua ativa, só não é chamada por nenhum ficheiro de `src/`. Portar segue o mesmo padrão desta secção: comprimir com `compressImage`, montar o payload, invocar via `invokeEdgeFunctionWithTimeout`, confirmar o enum de campos fixos (se existir) contra a Edge Function, e considerar o mesmo modo manual (sem imagem, só comentário do Coach) se a função ainda não o tiver.
